...
<head>
    ...
    <title>Diskon Menu - POS Admin</title>
        ...
        <main class="flex-1 p-6 md:p-8 lg:p-10 overflow-y-auto bg-gray-50">
            <div class="mb-8 mt-20">
                <div class="flex items-center justify-between">
                    <div>
                        <h2 class="text-3xl md:text-4xl font-bold text-indigo-700 flex items-center gap-4">
            ...
            <div class="bg-white rounded-2xl shadow-lg">
                <div class="p-6 border-b border-gray-200">
                    <form method="GET" class="flex gap-4 items-end">
                        <div class="flex-1">
                            <label class="block text-sm font-semibold text-gray-700 mb-2">Cari Menu</label>
                            <input type="text" name="search" value="{{ request('search') }}" placeholder="Nama menu..." class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500">
                        </div>
                        <div>
                            ...
                            @forelse($diskons as $diskon)
                                <tr class="hover:bg-gray-50 transition-colors">
                                    <td class="px-6 py-4 text-sm text-gray-900">
                                        <div class="font-semibold">{{ $diskon->menu->nama }}</div>
                                        <div class="text-xs text-gray-500">Rp {{ number_format($diskon->menu->harga_base, 0, ',', '.') }}</div>
                                    </td>
                                    <td class="px-6 py-4 text-sm text-gray-900">
                                    ...
                                    <td class="px-6 py-4 text-sm">
                                        <div class="flex gap-2">
                                            <a href="{{ route('admin.diskon.menu.edit', $diskon) }}" class="px-3 py-1 bg-indigo-600 hover:bg-indigo-700 text-white rounded text-xs font-semibold transition-colors">
                                                Edit
                                            </a>
                            ...
                            @empty
                                <tr>
                                    <td colspan="5" class="px-6 py-12 text-center text-gray-500">
                                        ...
                                        <p class="text-lg font-semibold">Tidak ada diskon menu</p>
                                    </td>
                                </tr>
                            @endforelse
                        </tbody>
